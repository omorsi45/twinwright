...
Open an in-memory SQLite Store and seed from the recorded seed and manifest digest. Create one replay run with the original run ID, task, provider, model, scenario, and one-shot fault, but the new world ID. The original run ID preserves deterministic refund IDs derived from run ID and call ID. Decode recorded assistant messages with JSON number preservation, then feed them through the existing Runner and Dispatcher with a provider that only returns those messages in order. There is no network call. Allow enough steps to complete, and ensure every recorded response is consumed.
...
The package lives at internal/replay and uses the existing provider, runner, dispatcher, compiler manifest, and store interfaces. Store gains a narrow method to create an isolated replay run with a fixed ID and a read-only opener for source databases. The CLI adds replay <run-id> --manifest PATH --db PATH. No source migration is required.
