...
func Handle(ctx context.Context, tx *sql.Tx, worldID, runID, callID, behavior string, args map[string]any) (int, any, *Mutation, error) {
	...
	case "billing.getCustomer":
		...
		if err == sql.ErrNoRows {
			return 404, map[string]string{"error": "customer not found"}, nil, nil
		}
		if err != nil {
	...
	case "billing.listInvoices":
		...
		out := []map[string]any{}
		for rows.Next() {
			var id, cust, sub string
			var amount int64
			if err = rows.Scan(&id, &cust, &amount, &sub); err != nil {
				return 0, nil, nil, err
			}
			out = append(out, map[string]any{"id": id, "customer_id": cust, "amount_cents": amount, "subscription_id": sub})
		}
	...
	case "billing.getCharge":
		...
		if err == sql.ErrNoRows {
			return 404, map[string]string{"error": "charge not found"}, nil, nil
		}
		if err != nil {
			return 0, nil, nil, err
		}
	...
	case "billing.createRefund":
		...
		if amount > charged-refunded {
			return 409, map[string]string{"error": "refund exceeds remaining charge"}, nil, nil
		}
		sum := sha256.Sum256([]byte(runID + ":" + callID))
		refundID := "RF-" + hex.EncodeToString(sum[:6])
		var base string
		...
		if _, err = tx.ExecContext(ctx, "INSERT INTO refunds(world_id,id,charge_id,amount_cents,reason,created_at) VALUES(?,?,?,?,?,?)", worldID, refundID, chargeID, amount, reason, at); err != nil {
			return 0, nil, nil, err
		}
		if _, err = tx.ExecContext(ctx, "UPDATE charges SET refunded_cents=refunded_cents+? WHERE world_id=? AND id=?", amount, worldID, chargeID); err != nil {
			return 0, nil, nil, err
		}
...
func scanCharge(row scanner) (map[string]any, error) {
	var id, invoice, created string
	var amount, refunded int64
	if err := row.Scan(&id, &invoice, &amount, &refunded, &created); err != nil {
		return nil, err
	}
