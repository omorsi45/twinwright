...
The first `standard` suite has 16 curated cases. Scripted fixtures are the verified CI path. Live providers may be selected with the same flags as `run`, and no live bench has been verified in this repository. Duplicate Effects in the summary is an incidence rate of bad outcomes among tagged cases, not a success rate.
