...
Forking requires a paused or completed source, a matching manifest, and a reproducible prefix. A changed prefix, tampered result or transcript, unsupported boundary, or failed transaction leaves no child. Only the currently implemented seeded scenarios and registered behavior handlers can be reconstructed. Replay of a completed fork must verify its parent prefix before checking the child suffix.
